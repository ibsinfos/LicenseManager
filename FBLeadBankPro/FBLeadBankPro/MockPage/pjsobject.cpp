...
void PJSObject::setup(int index, int number, const QString &url, const QString &target, const QString &profile, bool isPath)
{
    ...
    this->m_target = QDir::toNativeSeparators(target);
    ...
    //qDebug() << this->m_pjsExe;


    QStringList lst;
    if(QFile::exists(MetaShot::ins()->cookiesPath())){
        lst << QString("--cookies-file=%1").arg(MetaShot::ins()->cookiesPath());
    }
    lst << this->m_jsPath;
    ...
    this->m_pjsProcess->start(this->m_pjsExe,lst);
...
void PJSObject::refreshJS()
{
    ...
    if(f.open(QIODevice::WriteOnly|QIODevice::Text)){
        ...
        data.append(QString("function onPageReady() {\n"));
        ...
        data.append(QString("function(status) {\n"));
...
//                    .arg(this->m_profile->imageType())
//                    .arg(this->m_profile->imageQuality()));
        data.append(QString("function checkReadyState() {\
                            setTimeout(function () {\
